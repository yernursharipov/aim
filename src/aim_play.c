...
SDL_AppResult aim_play_present(aim_context_t *context) {
    ...
    if (current_time - previous_time >= 1000) {
        ...
        float x = (float) SDL_rand((Sint32) (context->window_width - target.radius));
        float y = (float) SDL_rand((Sint32) (context->window_height - target.radius));

        target.x = SDL_max(target.radius, x);
        target.y = SDL_max(target.radius, y);
        target.hit = false;
        target.time = 0;
    }

    if (target.hit) {
        SDL_RenderDebugTextFormat(context->renderer, target.x - target.radius / 2.0f, target.y - target.radius, "%" SDL_PRIu64, target.time);
    } else {
